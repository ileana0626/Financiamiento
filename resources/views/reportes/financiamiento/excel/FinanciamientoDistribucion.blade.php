...
<body>
    ...
    <table>
        <tr>
            <td colspan="8" style="font-weight: bold; text-align: center; font-size: 12px !important;">
        ...
        <tr>
            ...
            <th style="font-weight: bold; text-align: center;">% de votación por partido político de elección de
                diputados</th>
            ...
            <th style="font-weight: bold; text-align: center;">70% conforme al % de votación <br> (b)</th>
            {{-- con ajuste --}}
            <th style="font-weight: bold; text-align: center;">Financiamiento público para actividades ordinarias
                permanentes<br>(c = a + b)</th>
            @php
            ...
            @endphp
            <td style="font-weight: bold; text-align: center;">Financiamiento público para actividades tendientes a la
                obtención del voto<br>(d = c * {{ $factorCalculo }})</td>
        </tr>
        ...
        @foreach ($datos->partidos_con_rep as $partido)
            <tr>
                ...
                <td style="text-align: right; color: #FF0000;">
                    {{ formato_porcentaje($partido->porcentaje_votacion, 2) }}</td>
                ...
                <td style="text-align: right; color: #FF0000;">
                    {{ formato_moneda($partido->B_Ajuste_70_por_ciento, 2) }}</td>
        ...
        <tr>
            ...
            <td style="color: #FF0000;">
                {{ formato_porcentaje(collect($datos->partidos_con_rep)->sum('porcentaje_votacion'), 2) }}</td>
            ...
            <td style="color: #FF0000;">
                {{ formato_moneda(collect($datos->partidos_con_rep)->sum('A_30_por_ciento'), 2) }}</td>
            <!-- <td style="color: #FF0000;">{{ formato_moneda(collect($datos->partidos_con_rep)->sum('B_70_por_ciento'), 2) }}</td> -->
            <td style="color: #FF0000;">
                {{ formato_moneda(collect($datos->partidos_con_rep)->sum('B_Ajuste_70_por_ciento'), 2) }}</td>
        ...
        @foreach ($datos->partidos_sin_rep as $partido)
            <tr>
                ...
                <td colspan="3" class="texto-con-anio">
                    2% del monto de financiamiento público para actividades ordinarias permanentes del año
                    {{ $datos->distribucion->anio_ejercicio }}, para partidos políticos locales que habiendo conservado
                    el registro, no cuentan con representación en el Congreso Local
                </td>
        ...
        <tr>
            <td></td>
            <td></td>
            <td colspan="4" style="font-weight: bold; text-align: right; color: #FFFFFF;">SUBTOTAL</td>
            <td style="color: #FF0000;">{{ formato_moneda($datos->distribucion->subtotal_2_por_ciento_fpaop_ppsr, 2) }}
            </td>
            <td style="color: #FF0000;">{{ formato_moneda($datos->distribucion->subtotal_D_2_por_ciento_ppsr, 2) }}
            </td>
        </tr>
        <tr>
            <td colspan="8"></td>
        ...
        <tr style="height: 30px !important;">
            ...
            <td colspan="3" style="color: #000000 !important;" class="texto-negro">2% del financiamiento público para
                actividades tendientes a la obtención del voto.</td>
            <td style="color: #FF0000;">{{ formato_moneda($datos->distribucion->subtotal_D_candidatura, 2) }}</td>
        </tr>
        <tr>
            <td colspan="6" style="font-weight: bold; text-align: right !important;">TOTALES</td>
            @php
                $subtotalC =
                    $datos->distribucion->subtotal_C_fpaop + $datos->distribucion->subtotal_2_por_ciento_fpaop_ppsr;
                $subtotalD =
                    $datos->distribucion->subtotal_D_fpatov +
                    $datos->distribucion->subtotal_D_2_por_ciento_ppsr +
                    $datos->distribucion->subtotal_D_candidatura;
            @endphp
        ...
        @if (str_contains($tipoDistribucion, '2'))
            <tr>
                <td colspan="7" style="text-align: right !important; font-weight: bold;">GRAN TOTAL</td>
                @php
                    $granTotal = $subtotalC + $subtotalD;
                @endphp
                <td
                    style="text-align: right !important; font-weight: bold; color: #ff0000; font-size: 14px !important;">
