    ...
    <div class="col">
        ...
        <div class="row col-md-11 col-10 mx-auto card card-info  px-0"
            ...
            <div class="card-header">
                <h3 class="col card-title font-weight-bold">Edición de Solicitudes</h3>
            </div>
            <div class=" p-4 container-fluid" style="background-color: var(--iee-white) !important;">
                <div class="row mx-12 mb-12 p-4">
                    ...
                    <div class="col-sm-6 col-md-4 col-xl-3 px-0 pr-sm-5 pb-3" v-if="respuesta == 1">
                        <label class="col-form-label">Quien supervisa</label>
                        <vs-select placeholder="Seleccione una opción" v-model="supervisa"
                            v-if="cat_seguimiento.length > 0" :color="colors[0].color" filter autocomplete="off">
                            <template #message-danger v-if="errorSupervisa.length > 0" :disabled="bloqueo">
                                {{ errorSupervisa }}
                            </template>
                    ...
                    <div class="col-sm-6 col-md-4 col-xl-3 px-0 pr-sm-5 pb-5">
                    ...
                    <div class="row mx-12 mb-12 p-4">
                        <div class="col-sm-12 col-md-8 col-xl-6 px-0 pr-sm-5 pb-3">
                            <center>
                                <vs-button style="width: 19vw; " color="#a5904a" block @click.prevent="editarSolicitud">
    ...
    methods: {
        ...
        async editarSolicitud() {
            ...
            if (this.error == 1) {
                ...
                let url = '/administracion/usuario/editarSolicitud'
                axios.post(url, {
                    ...
                    this.guardarCopiasConocimiento(this.idSolicitud, loading)
                }).catch(error => {
                    loading.close();
                    let nombreMetodo = url.split('/');
                    methods.catchHandler(error, nombreMetodo[3]);
        ...
        validar() {
            ...
            if (this.supervisa.length == 0) {
                this.error = 1
                this.errorSupervisa = 'Seleccione la quién supervisa'
            }
        ...
        guardarCopiasConocimiento(id, loading) {
            ...
            axios.post(url, {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Registrado actualizado correctamente',
                    showConfirmButton: true,
                    confirmButtonText: 'De acuerdo'
                });
            }).catch((error) => {
        ...
        obtenerSolicitud() {
            ...
            let url = '/administracion/usuario/obtenerSolicitud'
            ...
            axios.get(url, {
                ...
                if (response.data.length > 0) {
                    this.nConsecutivo = response.data[0].numeroConsecutivo
                    this.nSolicitud = response.data[0].numeroSolicitud
                    this.fechaRecibido = response.data[0].fechaRecibido
                    this.remitente = response.data[0].remitente
                    this.otroRemitente = response.data[0].otroRemitente
                    this.cargo = response.data[0].cargo
                    this.otroCargo = response.data[0].otroCargo
                    this.asunto = response.data[0].asunto
                    this.termino = response.data[0].termino
                    this.fechaTermino = response.data[0].fechaTermino
                    this.diaTermino = response.data[0].diasTermino
                    this.respuesta = response.data[0].respuesta
                    this.seguimiento = response.data[0].seguimiento
                    this.areaAsignada = response.data[0].areaAsignada
                    this.tipo = response.data[0].tipo
                    this.fechaAsignacion = response.data[0].fechaAsignacion
                    this.estatus = response.data[0].estatus
                    this.observaciones = response.data[0].observaciones
                    this.supervisa = response.data[0].supervisa

                    this.obtenerCopiasConocimiento(this.idSolicitud, loading)
                }
                else {
                    loading.close();
                    Swal.fire({
                        icon: 'warning',
                        title: 'La solicitud se encuentra concluida', showConfirmButton: true,
                        confirmButtonText: 'De acuerdo'
                    }).then((result) => {
                        if (result.value) {
                            // this.$router.push({name: 'solicitudes.ver'})
                        }
                    })
                }
        ...
        obtenerCopiasConocimiento(id, loading) {
            let url = '/administracion/usuario/obtenerCopiasConocimiento'
            axios.get(url, {
                params: {
                    'idSolicitud': id
                }
                ...
                loading.close();
                if (response.data[0].departamentosEnterados != null) {
                    this.copiasConocimiento = response.data[0].departamentosEnterados
                }
            }).catch(error => {
                loading.close();
                let nombreMetodo = url.split('/');
